...
const soldadoUm = document.querySelector("#soldado1");
const soldadoDois = document.querySelector("#soldado2");
let umAtivo = false;
let doisAtivo = false;
let vidaSoldadoUm = 10;
let vidaSoldadoDois = 10;
...
/*
const tamBau = (".bau");   
const dimensoesbau = tamBau.getBoundingClientRect();
const tamanhoMinimoX = limiteBaus * dimensoesbau; // Tamanho mínimo em pixels

if (window.innerWidth < tamanhoMinimo || window.innerHeight < tamanhoMinimo) {
    alert("A tela é muito pequena para gerar os itens.");
    return;
}*/
...
let inimigosRun = [
    "assets/BuzzyLabs/buzzy-bee.svg",
    "assets/BuzzyLabs/buzzy-bee2.svg",
];
...
function gerarCoisas() {
    ...
    const limiteBarreiras = Math.floor(Math.random() * 2) + 1;
    const limiteSujeira = Math.floor(Math.random() * 10) + 2;
    // Lista de itens existentes para verificar colisões
    let itensExistentes = [];

    const gerarUm = Math.floor(Math.random() * 2);
    const gerarDois = Math.floor(Math.random() * 2);
    ...
    soldadoDois.style.top = `${yDois}px`;
    soldadoDois.style.left = `${xDois}px`;
    soldadoUm.style.top = `${yUm}px`;
    soldadoUm.style.left = `${xUm}px`;
    ...
    for (let i = 0; i < limiteBaus; i++) {
        ...
        novoBau.src = 'assets/BuzzyLabs/buzzy-chest.svg';
    ...
    for (let i = 0; i < limiteBarreiras; i++) {
        ...
        novaBarreira.src = 'assets/BuzzyLabs/buzzy-trap.svg';
    ...
    for (let i = 0; i < limiteInimigos; i++) {
        ...
        novoInimigo.src = 'assets/BuzzyLabs/buzzy-bee.svg';
    ...
    for (let i = 0; i < limiteSujeira; i++) {
        ...
        novaSujeira.src = 'assets/BuzzyLabs/buzzy-honey.svg';
...
document.querySelector("#pilar").addEventListener("click", () => {
    if(todosBausAbertos){
        ...
        setTimeout(() => {
            bodyEl.style.backgroundImage = "url('assets/BuzzyLabs/buzzy-floor.svg')";
            gerarCoisas();
...
let intervalosDiversos = setInterval(() => {
    ...
    if (detectaColisaoUnica(hitBoxEl, soldadoDois) && tomarDano) {
        darDanoJogador();
    }

    baus.forEach((bau, i) => {
        if (detectaColisaoUnica(bau, playerEl)) {
            bau.src = 'assets/BuzzyLabs/buzzy-chest-open.svg';  
    ...
    if(todosZero){
        document.querySelector("#pilar img").src ="assets/BuzzyLabs/buzzy-pilar-open.svg";
        todosBausAbertos = true;
    }
    else{
        document.querySelector("#pilar img").src ="assets/BuzzyLabs/buzzy-pilar.svg";
        todosBausAbertos = false;
    }
